...
export class GIAPDeviceInfo {
  ...
  static getBrowserVersion = (userAgent, vendor, opera) => {
    const browser = GIAPDeviceInfo.getBrowser(userAgent, vendor, opera);
    const versionRegexs = {
...
export default (props, windowCurrent = window) => {
  ...
  res.browser = GIAPDeviceInfo.getBrowser(userAgent, vendor, opera);
  res.browserVersion = GIAPDeviceInfo.getBrowserVersion(userAgent, vendor, opera);
  res.currentUrl = location.href;
  res.os = GIAPDeviceInfo.getOs(userAgent);
  res.referrer = referrer;
  res.referringDomain = GIAPDeviceInfo.getReferringDomain(referrer);
  res.searchEngine = GIAPDeviceInfo.getSearchEngine(referrer);
  res.device = GIAPDeviceInfo.getDevice(userAgent);
  return res;
};
